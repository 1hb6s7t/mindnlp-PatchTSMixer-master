...
def threads_exclusive_http_get(url, storage_folder=None, md5sum=None, download_file_name=None, proxies=None, headers=None):
    ...
    try:
        ...
        with portalocker.Lock(lock_file_path, timeout=10, mode='w') as lock:
            # 检查文件是否已存在
            if os.path.exists(pointer_path):
                return pointer_path
                
            # 执行下载操作
            from mindnlp.utils import http_get  # 延迟导入避免循环依赖
            file_path = http_get(
                url, 
                path=storage_folder,
        ...
        raise RuntimeError(f"Failed to acquire lock after 10 seconds: {lock_file_path}") from e
